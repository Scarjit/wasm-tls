...
use std::{io::{Read, Write, BufReader}, sync::{Arc, Mutex}};
...
pub fn create_server() -> Result<(), JsValue> {
    ...
    let key = match rustls_pemfile::pkcs8_private_keys(&mut key_reader).next() {
        ...
        _ => return Err(JsValue::from_str("[create_server] Failed to parse private key")),
    };
    ...
    let server_conn = ServerConnection::new(Arc::new(config))
        .map_err(|e| JsValue::from_str(&format!("[create_server] Server connection error: {}", e)))?;
...
pub fn server_send_data(data: &[u8]) -> Result<(), JsValue> {
    ...
    let server = server_lock.as_mut().ok_or_else(|| JsValue::from_str("[server_send_data] Server not initialized"))?;
...
pub fn server_get_data() -> Result<Vec<u8>, JsValue> {
    let mut server_lock = SERVER.lock().unwrap();
    let server = server_lock.as_mut().ok_or_else(|| JsValue::from_str("[server_get_data] Server not initialized"))?;
...
pub fn server_process_tls() -> Result<(), JsValue> {
    let mut server_lock = SERVER.lock().unwrap();
    let server = server_lock.as_mut().ok_or_else(|| JsValue::from_str("[server_process_tls] Server not initialized"))?;
    
    // Process data based on TLS state
    if server.server.wants_read() && server.socket.has_data_to_read() {
        server.server.read_tls(&mut server.socket)
            .map_err(|e| JsValue::from_str(&format!("[server_process_tls] Read error: {}", e)))?;
        
        let state = server.server.process_new_packets()
            .map_err(|e| JsValue::from_str(&format!("[server_process_tls] Process error: {}", e)))?;
        server.available_bytes = state.plaintext_bytes_to_read();
    }
    
    if server.server.wants_write() {
        server.server.write_tls(&mut server.socket)
...
pub fn server_read_request() -> Result<String, JsValue> {
    let mut server_lock = SERVER.lock().unwrap();
    let server = server_lock.as_mut().ok_or_else(|| JsValue::from_str("[server_read_request] Server not initialized"))?;
    
    let mut plaintext = Vec::with_capacity(server.available_bytes);
    let reader = server.server.reader();
    let n = reader.take(server.available_bytes as u64).read_to_end(&mut plaintext)
    ...
    if n != server.available_bytes {
        return Err(JsValue::from_str("[server_read_request] Read error: n != server.available_bytes"));
    }
...
pub fn server_write_response(response: &str) -> Result<(), JsValue> {
    let mut server_lock = SERVER.lock().unwrap();
    let server = server_lock.as_mut().ok_or_else(|| JsValue::from_str("[server_write_response] Server not initialized"))?;
