...
use rustls::{ClientConfig, RootCertStore, pki_types::TrustAnchor};
...
pub fn send_data(data: &[u8]) -> Result<(), JsValue> {
    ...
    let client = client_lock.as_mut().ok_or_else(|| JsValue::from_str("[send_data] Client not initialized"))?;
...
pub fn get_data() -> Result<Vec<u8>, JsValue> {
    let mut client_lock = CLIENT.lock().unwrap();
    let client = client_lock.as_mut().ok_or_else(|| JsValue::from_str("[get_data] Client not initialized"))?;
...
pub fn process_tls() -> Result<(), JsValue> {
    let mut client_lock = CLIENT.lock().unwrap();
    let client = client_lock.as_mut().ok_or_else(|| JsValue::from_str("[process_tls] Client not initialized"))?;
    
    // Process data based on the example provided
    if client.client.wants_read() && client.socket.has_data_to_read() {
        client.client.read_tls(&mut client.socket)
            .map_err(|e| JsValue::from_str(&format!("[process_tls] Read error: {}", e)))?;
        
        let state = client.client.process_new_packets()
    ...
    if client.client.wants_write() {
        client.client.write_tls(&mut client.socket)
...
pub fn write_request(request: &str) -> Result<(), JsValue> {
    let mut client_lock = CLIENT.lock().unwrap();
    let client = client_lock.as_mut().ok_or_else(|| JsValue::from_str("[write_request] Client not initialized"))?;
...
pub fn read_response() -> Result<String, JsValue> {
    let mut client_lock = CLIENT.lock().unwrap();
    let client = client_lock.as_mut().ok_or_else(|| JsValue::from_str("[read_response] Client not initialized"))?;
    
    let mut plaintext = Vec::with_capacity(client.available_bytes);
    let reader = client.client.reader();
    let n = reader.take(client.available_bytes as u64).read_to_end(&mut plaintext)
    ...
    if n != client.available_bytes {
        return Err(JsValue::from_str("[read_response] Read error: n != client.available_bytes"));
    }
